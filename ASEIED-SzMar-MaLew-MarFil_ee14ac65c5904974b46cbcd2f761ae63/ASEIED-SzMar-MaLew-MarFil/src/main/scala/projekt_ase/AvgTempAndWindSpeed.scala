...
class AvgTempAndWindSpeed {
  ...
  val sparkSession = SparkSession.builder.
    ...
    def avgTemp(path: String): Unit = {
        ...
        val data1 = weather_data.select(col(" Avg Temp").plus(0).name("AvgTemp"), col(" Min Temp").plus(0).name("MinTemp"), col(" Max Temp").plus(0).name("MaxTemp"))
    ...
    def avgWindSpeedDaily(path: String): Unit = {
        val weather_data = sparkSession.read.option("delimiter", ",").option("header","true").csv(path)
        val data1 = weather_data.select(col(" Wind Speed").plus(0).name("WindSpeed"), col(" Wind Avg Speed").plus(0).name("WindAvgSpeed"))
    ...
    def avgWindSpeedHourly(path: String): Unit = {
        val weather_data = sparkSession.read.option("delimiter", ",").option("header","true").csv(path)
        val data1 = weather_data.select(col(" Wind Speed (kt)").plus(0).name("WindSpeedKT"))
